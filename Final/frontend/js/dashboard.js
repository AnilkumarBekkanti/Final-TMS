...
function filterTasks(tasks) {
    ...
    return tasks.filter(task => {
        ...
        // Date range filter
        if (startDate && task.deadline < startDate) {
            return false;
        }
        if (endDate && task.deadline > endDate) {
            return false;
        }
...
// Update the fetchTasks function to store original tasks
let allTasks = []; // Add this at the top of your file
...
async function fetchTasks() {
    ...
    try {
        ...
        allTasks = await response.json(); // Store all tasks
        const filteredTasks = filterTasks(allTasks); // Apply filters
...
// Add search input to event listeners
document.getElementById('searchInput').addEventListener('input', function(e) {
    const searchResults = searchTasks(e.target.value);
    const filteredResults = filterTasks(searchResults);
    displayTasks(filteredResults);
    updateDashboardStats(filteredResults);
});
